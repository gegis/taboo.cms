...
class ModuleCommand {
  ...
  async deleteFilesByCmsType(cmsType, moduleNameUpper, moduleNameLower) {
    ...
    if (cmsType === 'headless') {
      await this.cliHelper.deleteFile(path.resolve(newModulePath, 'client', 'app.config.js'));
      await this.cliHelper.deleteFile(path.resolve(newModulePath, 'client/styles', 'index.less'));
    }
  }
}
