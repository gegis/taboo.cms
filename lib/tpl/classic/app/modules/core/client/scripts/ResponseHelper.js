...
class ResponseHelper {
  handleError(err) {
    ...
    if (message === 'Not Verified' && errResponseStatus === 403) {
      ...
      Notifications.show(message, { type: 'error', delay: 10000 });
    }
  }

  handleValidationErrors(validationMessage, validationErrors) {
    validationErrors.map(validationError => {
      Notifications.show(validationError.message, { type: 'error', delay: 10000 });
    });
  }

  handleModelErrors(errors) {
    Object.keys(errors).forEach(field => {
      Notifications.show(`<b>${errors[field].path}</b> ${errors[field].message}`, { type: 'error', delay: 10000 });
    });
  }
