import Loader from 'modules/core/ui/scripts/Loader';
import Notifications from 'modules/core/ui/scripts/Notifications';
import ResponseHelper from 'modules/core/ui/scripts/ResponseHelper';
import Countries from 'modules/countries/ui/scripts/Countries';
import ArrayHelper from 'modules/core/ui/helpers/ArrayHelper';
  ...
  bindSignUpSubmit() {
    $('#sign-up-form').submit(event => {
      ...
      if (data.password !== data.passwordRepeat) {
        Notifications.show("Passwords don't match", { type: 'error', delay: 10000 });
      } else if (!data.userAgreement) {
        Notifications.show('You have to agree with terms and conditions', { type: 'error', delay: 10000 });
      } else {
        Loader.show();
        UserActions.register(data, (err, user) => {
          ...
          if (err) {
            ...
            Notifications.show('Sign Up has failed', { type: 'error', delay: 10000 });
          }
        });
      }
    });
    ...
    $('#my-profile-form').submit(event => {
      ...
      UserActions.updateMyProfile(data, (err, user) => {
        ...
        if (err) {
          ...
          Notifications.show('Data update has failed', { type: 'error', delay: 10000 });
        }
      });
    });
  }
    ...
    $('#reset-password-form').submit(event => {
      ...
      UserActions.resetPassword(email, (err, data) => {
        ...
        if (err) {
          ...
          Notifications.show('Password reset information has been successfully sent to your email address.', {
            type: 'info',
            delay: 10000,
          });
        } else {
          Notifications.show('Password reset has failed', { type: 'error', delay: 10000 });
        }
      });
    });
  }
      ...
      if (newPass !== newPassRepeat) {
        ...
        UserActions.changePassword(data, (err, data) => {
          ...
          if (err) {
            ...
            Notifications.show('Password change has failed', { type: 'error', delay: 10000 });
          }
        });
      }
    });
