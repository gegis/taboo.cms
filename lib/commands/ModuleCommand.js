...
class ModuleCommand {
  ...
  // TODO - move this cli command into application cli module (also new module template)
  async createModule(options) {
    ...
    if (type === 'classic') {
      await this.cliHelper.copyFolder(
        path.resolve(cmsPackageDir, 'lib/tpl/newModuleClassic/ui'),
        path.resolve(installDir, `app/modules/${moduleNameLower}/ui`)
      );
      await this.cliHelper.copyFolder(
  ...
  getNewModuleFileRenameOptions(cmsType, moduleNameUpper, moduleNameLower, modelNameUpper) {
    ...
    const options = [
      ...
      {
        from: path.resolve(newModulePath, 'ui/components/admin', 'CreateModelNameModal.jsx'),
        to: path.resolve(newModulePath, 'ui/components/admin', `Create${modelNameUpper}Modal.jsx`),
      },
      {
        from: path.resolve(newModulePath, 'ui/components/admin', 'EditModelNameModal.jsx'),
        to: path.resolve(newModulePath, 'ui/components/admin', `Edit${modelNameUpper}Modal.jsx`),
      },
      {
        from: path.resolve(newModulePath, 'ui/components/admin', 'ModelNameForm.jsx'),
        to: path.resolve(newModulePath, 'ui/components/admin', `${modelNameUpper}Form.jsx`),
      },
      {
        from: path.resolve(newModulePath, 'ui/components/admin', 'ModuleName.jsx'),
        to: path.resolve(newModulePath, 'ui/components/admin', `${moduleNameUpper}.jsx`),
      },
      {
        from: path.resolve(newModulePath, 'ui/components/admin', 'ModuleNameList.jsx'),
        to: path.resolve(newModulePath, 'ui/components/admin', `${moduleNameUpper}List.jsx`),
      },
      {
        from: path.resolve(newModulePath, 'ui/components', 'ModuleName.jsx'),
        to: path.resolve(newModulePath, 'ui/components', `${moduleNameUpper}.jsx`),
      },
      {
        from: path.resolve(newModulePath, 'ui/stores', 'ModuleNameAdminStore.js'),
        to: path.resolve(newModulePath, 'ui/stores', `${moduleNameUpper}AdminStore.js`),
      },
      {
        from: path.resolve(newModulePath, 'ui/stores', 'ModuleNameStore.js'),
        to: path.resolve(newModulePath, 'ui/stores', `${moduleNameUpper}Store.js`),
      },
    ];
    if (cmsType === 'classic') {
      options.push({
        from: path.resolve(newModulePath, 'ui/scripts', 'ModuleName.js'),
        to: path.resolve(newModulePath, 'ui/scripts', `${moduleNameUpper}.js`),
      });
    }
    return options;
  }
  ...
  async deleteFilesByCmsType(cmsType, moduleNameUpper, moduleNameLower) {
    ...
    const newModulePath = path.resolve(installDir, `app/modules/${moduleNameLower}/`);
    if (cmsType === 'classic') {
      await this.cliHelper.deleteFile(path.resolve(newModulePath, 'ui/components', `${moduleNameUpper}.jsx`));
    }
  }
}
